...
const sketch = () => {
  return ({ context, width, height }) => {
    ...
    for (let i = 0; i < num; i++) {
      ...
      x = cx + random.range(radius - 80, width - 30) * Math.sin(angle);
      y = cy + random.range(radius - 80, height - 30) * Math.cos(angle);

      context.save();
      context.translate(x, y);
      context.rotate(-angle);
      // takes morphed element and changes its size
      context.scale(random.range(0.5, 3), random.range(0.7, 2));
      ...
      context.arc(
        0,
        0,
        random.range(radius - 70, width - 30),
        0,
        random.range(2, 17)
      );
      // context.lineWidth(random.range(1, 4));
      context.lineWidth = random.range(0.1, 4);
      context.stroke();
      context.restore();
    }
  };
